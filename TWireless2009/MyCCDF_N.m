function [CCDF_prec_u,CCDF_prec_g,CCDF_RayleighApp]=MyCCDF_N(N,P0,V,phi)
...
m_x_u = 2*(besselj(1,alpha_u)/alpha_u)*sqrt(N(cntr));
var_x_u = 0.5*(1+(besselj(1,2*alpha_u)/alpha_u))-(2*(besselj(1,alpha_u)/alpha_u))^2;
var_y_u = 0.5*(1-(besselj(1,2*alpha_u)/alpha_u));

m_x_g = sqrt(N(cntr))*exp(-((alpha_g^2)*V)/2);
var_x_g = 0.5*(1+ exp(-(((2*alpha_g)^2)*V)/2) )-(exp(-((alpha_g^2)*V)/2))^2;
var_y_g = 0.5*(1- exp(-(((2*alpha_g)^2)*V)/2) );
